    ...
    async resigter(req,res){
        ...
        try {
            ...
            bcrypt.hash(PassWord,10,async function(error,hash){
                if(!error){
                    const NewUser =new User({userName:UserName,userPassword:hash,tokenUserInfo:""});
                    NewUser.save();
                    const acessToken =jwt.sign({userId: NewUser._id,userFullName: FullName,userEmail: Email,userAvatar: Avatar},process.env.Token_Secret);
                    await User.findByIdAndUpdate(NewUser._id,{tokenUserInfo:acessToken});
                    return res.json({success: true, message:"Resigter success access Token is",acessToken});
                }else{
    ...
    async login(req,res){
        try {
            ...
            if(user.tokenUserInfo){
                ...
                return res.json({success: true, message:"Login Success",data:{idUser:decode.userId
                    , FullName:decode.userFullName, Email:decode.userEmail, Avatar: decode.userAvatar}});
            }
        } catch (error) {
            console.log(error);
            return res.json({success: false, message: 'Erorr !!'});
